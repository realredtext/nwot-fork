...
async function idToUser(id) {
	...
	var associatedUser = await db.get("SELECT username FROM auth_user WHERE id=?", id);
...
async function g() {
	superuserName = await db.get("SELECT username FROM auth_user WHERE id=1");
	prompt_command_input = {
		properties: {
			input: {
				message: ">>"+"\x1b[92;1m"+superuserName.username+"\x1b[96;1m@OWOT"+"\x1b[0;0m:"
			}
		}
	};
};
...
var url_regexp = [ // regexp , function/redirect to , options
	...
	[/^administrator\/world_search[\/]?$/g, pages.admin.world_search],
...
async function can_view_world(world, user) {
	...
	var is_owner = (world.owner_id == user.id) || user.operator || world.name == "ffa";
	...
	if(worldname.length == 1) { // regular world names
		...
		if(world.owner_id == null || (rename_casing && world.id == world_id)) {
			...
			if(!user.superuser) {
				...
				var oldOwner = await db.get("SELECT username FROM auth_user WHERE id=?", world.owner_id);
